################################################################################
# AB Experiment - Simple MNL, pooled AB_5 + AB_U
...
current_script <- function() {
  ...
  if (file.exists("MNL_simple_both.R")) {
    return(normalizePath("MNL_simple_both.R", mustWork = TRUE))
  }
  if (file.exists(file.path("AB_Experiment", "MNL_simple_both.R"))) {
    return(normalizePath(file.path("AB_Experiment", "MNL_simple_both.R"), mustWork = TRUE))
  }
  normalizePath("MNL_simple_both.R", mustWork = FALSE)
}
...
message("Estimating MNL_simple_both for pooled AB_5 + AB_U")

apollo_initialise()

apollo_control <- list(
  modelName = "MNL_simple_both",
  modelDescr = "AB simple MNL, pooled AB_5 + AB_U, first 200 kept trials, no heterogeneity, panel likelihood",
  indivID = "PROLIFIC_PID",
  outputDirectory = file.path(script_dir, "MNL_simple_both"),
  workInLogs = TRUE,
  nCores = 5
)
