...
let expName = 'Transports-AU';  // from the Builder filename that created this script
let expInfo = {
...
function trialRoutineEnd(snapshot) {
  return async function () {
    ...
    // --- LÓGICA DE SHUFFLE PARA ABA ---
    if (phase === 2) {
        // Si acabamos de entrar a la fase 2, inicializamos el primer trigger
        if (nextShuffleTrial === -1) {
            // util.randint(1, 11) genera un entero entre 1 y 10
            nextShuffleTrial = nLoop + util.randint(1, 11);
        }

        // Si el trial actual coincide con el trial agendado para shuffle
        if (nLoop === nextShuffleTrial) {
            util.shuffle(locations);
            console.log("Locations shuffled (Random Interval) in trial: " + nLoop);
            shuffleStatus = "TRUE";
            
            // Agendamos el siguiente shuffle (entre 1 y 10 trials después)
            nextShuffleTrial = nLoop + util.randint(1, 11);
        }
    }
...
const CANCEL_URL = "https://app.prolific.com/submissions/complete?cc=C6BUL6WT";
...
//-  if (isCompleted) {
//-    setTimeout(() => { window.location.href = COMPLETION_URL; }, 5000);
//-  }
//-
//-  return Scheduler.Event.QUIT;
//-}
